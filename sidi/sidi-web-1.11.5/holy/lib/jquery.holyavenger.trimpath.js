...
(function($) {
	...
	function getTemplateTarget(context, template){
		if(context.target){
		...
		if (!selector) {
			throw "<template> requires id or selector attribute or target";
		}
	...
	$.holyavenger.parseTemplate = function(template, context, callback) {
		template = $(template);
		var targetElement = getTemplateTarget(context, template);
		context.page = targetElement;
